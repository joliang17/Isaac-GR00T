...
class HybridEmbedding(nn.Module):
    """
    An embedding module that uses a frozen base embedding table and two new,
    trainable special embedding tables (A and B).
    """
    ...
    def _load_from_state_dict(
        ...
        (This logic remains unchanged as it's for backward compatibility)
        """
...
class HybridLMHead(nn.Module):
    """
    An LM head that uses a frozen base head and two new, trainable special heads.
    """
    # MODIFIED: Updated __init__ signature
    def __init__(self, 
                 base_head: nn.Linear, 
...
class EagleBackbone(nn.Module):
    ...
    def __init__(
        ...
        # MODIFIED: Add tuning flags for special token groups
        ...
        # MODIFIED: Helper function to get ID tensors and lookup tables
        def get_ids_and_lookup(tokens_to_add_list, base_vocab_size, total_vocab_size):
        ...
        special_ids_A, special_lookup_A, num_new_A = get_ids_and_lookup(to_add_A, base_vocab_size, total_vocab_size)
        special_ids_B, special_lookup_B, num_new_B = get_ids_and_lookup(to_add_B, base_vocab_size, total_vocab_size)
        ...
        if num_new_A > 0 or num_new_B > 0:
            ...
            hybrid_embedding = HybridEmbedding(
                base_embedding, 
                num_new_A, special_lookup_A,
                num_new_B, special_lookup_B
            )
            hybrid_lm_head = HybridLMHead(
                base_lm_head, 
                num_new_A, self.special_token_ids_A,
                num_new_B, self.special_token_ids_B,
                total_vocab_size
            )
            ...
            # 4. Tie the new special heads to the new special embeddings
            self.tie_special_weights()
        else:
            # Original fallback, though buffers for A/B will just be empty
        ...
        maybe_image_tokens = ["<IMG_CONTEXT>", "<img>", "</img>", ]
        img_ids = []
        ...
        # MODIFIED: Pass new flags to set_trainable_parameters
    ...
    def forward_eagle(self, vl_input: BatchFeature, past_key_values=None) -> BatchFeature:
        ...
        eagle_input.pop('llm_labels', None) # MODIFIED: Use pop with default
    ...
    def split_by_img_id(self, vl_input, eagle_logits: torch.Tensor, eagle_mask: torch.Tensor):
        # (This function appears correct and does not depend on hybrid internals)
        eagle_input = {
            k.removeprefix("eagle_"): v
            for k, v in vl_input.items()
    ...
    def flatten_actions(self, list_embeds, list_masks):
        ...
        if len(list_embeds) > 0:
            embeds_tensor = pad_sequence(list_embeds, batch_first=True)
            masks_tensor = pad_sequence(list_masks, batch_first=True)
        else:
            embeds_tensor = None
            masks_tensor = None
    ...
    def forward(self, vl_input: BatchFeature, past_key_values=None) -> BatchFeature:
        ...
        return BatchFeature(
            data={
                ...
                "backbone_features_multi":       eagle_embeds_multi,
                "backbone_attention_mask_multi": eagle_mask_multi,
