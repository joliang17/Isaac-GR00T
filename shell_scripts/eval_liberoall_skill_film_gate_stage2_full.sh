...
# liberoall_256_full_skill_router_film_gate_stage2.
#   array task 0 -> LIBERO-10 standard eval
#   array task 1 -> LIBERO-PRO eval (object / semantic / task perturbations)

#SBATCH --array=0-1
#SBATCH --job-name=eval_film_gate_s2
#SBATCH --output=slurm_output/eval_liberoall_skill_film_gate_stage2_%A_%a.log
#SBATCH --error=slurm_output/eval_liberoall_skill_film_gate_stage2_%A_%a.log
...
LABEL="liberoall_256_full_skill_router_film_gate_stage2"
EXPECTED_STEP=60000
...
EVAL_TAG="${EVAL_TAG:-gateprobe}"
GATE_USED_THRESHOLD="${GATE_USED_THRESHOLD:-0.1}"
GATE_PROBE_ARGS="--gate_probe --gate_used_threshold ${GATE_USED_THRESHOLD} --eval_tag ${EVAL_TAG}"
