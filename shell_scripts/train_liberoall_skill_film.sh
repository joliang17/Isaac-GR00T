...
# Exp B: FiLM skill conditioning (--use_skill_film)
#   action_features = action_features * (1 + gamma) + beta,  (gamma, beta) = skill_film(skill_emb)
#   layout = [state(1) | future(32) | actions(T)]  (no separate skill token)
# Tests whether modulating every action token with the skill embedding
# increases the influence of the skill signal.

#SBATCH --job-name=liberoall_skill_film
#SBATCH --output=slurm_output/liberoall_skill_film.log
#SBATCH --error=slurm_output/liberoall_skill_film.log
...
STAGE1_NAME="liberoall_256_full_skill_router_film_stage1"
STAGE2_NAME="liberoall_256_full_skill_router_film_stage2"
